...
function inputLabel(activity: ToolCallActivity): string {
  ...
  switch (activity.tool) {
    ...
    case 'web_search': return `web_search(${JSON.stringify(input.query ?? '')})`
    case 'read_context': return `read_context(${input.files ?? 0} file${input.files === 1 ? '' : 's'})`
  }
}
...
function runningLabel(tool: ToolCallActivity['tool']): string {
  switch (tool) {
    ...
    case 'read_context': return 'reading selected context'
  }
}
