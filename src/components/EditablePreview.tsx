...
export default function EditablePreview({ proposal, onWritten, onDiscard }: {
  ...
  onWritten: (path: string, actionItems?: string[]) => void
  onDiscard: () => void
}) {
  const [content, setContent] = useState(proposal.content)
  const [selectedActions, setSelectedActions] = useState<string[]>(proposal.actionItems ?? [])
  ...
  useEffect(() => {
    ...
    const closeOnEscape = (event: KeyboardEvent) => { if (event.key === 'Escape' && !writing) onDiscard() }
  ...
  async function approve(): Promise<void> {
    ...
    try {
      ...
      if (result.ok) onWritten(result.path ?? proposal.path, selectedActions)
  ...
  return (
    <section className="editable-preview" aria-label="Proposed note">
      ...
      {proposal.actionItems && proposal.actionItems.length > 0 && (
        <div className="preview-action-items">
          <p className="caption-label">Extract tasks to Review Queue</p>
          {proposal.actionItems.map((item, index) => (
            <label key={`${index}-${item}`} className="action-item-checkbox">
              <input type="checkbox" checked={selectedActions.includes(item)} onChange={(e) => {
                setSelectedActions(curr => e.target.checked ? [...curr, item] : curr.filter(x => x !== item))
              }} />
              <span>{item}</span>
            </label>
          ))}
      ...
      <textarea ref={editor} id="preview-content" className="preview-content" value={content} onChange={(event) => setContent(event.target.value)} spellCheck={false} rows={14} disabled={writing} />
