...
**One-liner:** Noema is an autonomous knowledge agent for Obsidian vaults — a second brain that works *for* you, not just a place that stores notes.
...
**The solution:** Noema opens a real folder on disk (the user's actual Obsidian vault), indexes it, and gives an LLM agent tools to read, search, write, and link real `.md` files in that vault — with every generated claim traceable back to a real note.
...
| **F4** | Ask-your-knowledge | Grounded Q&A over the vault. Cites sources. **Refuses to answer** rather than hallucinate if nothing relevant is found in the index. | Must-have |
