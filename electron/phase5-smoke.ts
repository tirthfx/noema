...
// Capturing a local or private-network page could leak device-local data to the model.
const privateUrl = await proposeCapture(vault, { kind: 'url', value: 'http://127.0.0.1/private' }, () => {})
assert(!privateUrl.proposal && privateUrl.error?.includes('private network'), `Expected a private URL rejection, got: ${JSON.stringify(privateUrl)}`)
console.log('URL capture rejects private-network addresses before fetching.')

await getVaultIndex(vault).refresh()

// 1. Text capture proposes a draft and writes nothing.
const captured = await proposeCapture(vault, { kind: 'text', value: 'Interleaving different problem types during study improves the learner ability to discriminate between problem categories. Blocked practice feels easier but produces weaker discrimination.' }, () => {})
