...
import { readVaultNote, walkMarkdownFiles, writeVaultNote } from './vault'
...
await mkdir(join(vault, '.agents'), { recursive: true })
await mkdir(join(vault, 'node_modules', 'fixture'), { recursive: true })
await mkdir(join(vault, 'Knowledge'), { recursive: true })
await writeFile(join(vault, '.agents', 'private.md'), '# Hidden machinery\n')
await writeFile(join(vault, 'node_modules', 'fixture', 'package.md'), '# Generated dependency\n')
await writeFile(join(vault, 'Knowledge', 'kept.md'), '# User knowledge\n')
const walked = await walkMarkdownFiles(vault)
assert(walked.some((note) => note.path === join('Knowledge', 'kept.md')), 'Expected normal nested knowledge notes to be indexed.')
assert(!walked.some((note) => note.path.includes('.agents') || note.path.includes('node_modules')), 'Hidden and generated directories must not enter the knowledge index.')
